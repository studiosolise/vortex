-- ============================================================
-- VORTEX — Atualização do Banco de Dados
-- Execute no Supabase SQL Editor
-- ============================================================

-- ============================================================
-- 1. ADICIONAR COLUNA content NA TABELA lessons
-- ============================================================
ALTER TABLE lessons ADD COLUMN IF NOT EXISTS content TEXT;

-- ============================================================
-- 2. ADICIONAR COLUNAS EM project_completions
-- ============================================================
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'pending';
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS submission_url TEXT;
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS submission_notes TEXT;
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS admin_feedback TEXT;
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS admin_reviewed_at TIMESTAMPTZ;

-- Migrar registros antigos: quem já tinha completed_at = approved
UPDATE project_completions SET status = 'approved' WHERE status IS NULL AND completed_at IS NOT NULL;

-- ============================================================
-- 2b. CORRIGIR SCHEMA PARA SUPORTE MULTI-USUÁRIO
-- CAUSA RAIZ dos erros "Erro ao salvar nota" e "Erro ao salvar progresso":
--   1. As tabelas progress e unlocked_achievements tinham PRIMARY KEY simples
--      (lesson_id e achievement_id) — impossível ter mais de um usuário com progresso.
--   2. Nenhuma dessas tabelas tinha coluna user_id, mas o código JS insere/filtra por ela.
-- ============================================================

-- progress: remover PK simples (lesson_id) e recriar com PK composta (user_id, lesson_id)
...
-- Adiciona PK composta só se ainda não existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'progress' AND constraint_type = 'PRIMARY KEY'
  ) THEN
    ALTER TABLE progress ADD PRIMARY KEY (user_id, lesson_id);
  END IF;
END $$;

-- unlocked_achievements: remover PK simples (achievement_id) e recriar com PK composta
ALTER TABLE unlocked_achievements DROP CONSTRAINT IF EXISTS unlocked_achievements_pkey;
ALTER TABLE unlocked_achievements ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'unlocked_achievements' AND constraint_type = 'PRIMARY KEY'
  ) THEN
    ALTER TABLE unlocked_achievements ADD PRIMARY KEY (user_id, achievement_id);
  END IF;
END $$;

-- Adicionar user_id nas demais tabelas que também não tinham
...
ALTER TABLE project_completions ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
-- modules: leitura para qualquer usuário autenticado
DROP POLICY IF EXISTS "modules_read" ON modules;
DROP POLICY IF EXISTS "authenticated_read" ON modules;
CREATE POLICY "authenticated_read" ON modules
  FOR SELECT
  TO authenticated
  USING (true);

-- lessons: leitura para qualquer usuário autenticado
DROP POLICY IF EXISTS "lessons_read" ON lessons;
DROP POLICY IF EXISTS "authenticated_read" ON lessons;
CREATE POLICY "authenticated_read" ON lessons
  FOR SELECT
  TO authenticated
  USING (true);

-- projects: leitura para qualquer usuário autenticado
...
-- achievements: leitura para qualquer usuário autenticado
DROP POLICY IF EXISTS "achievements_read" ON achievements;
DROP POLICY IF EXISTS "authenticated_read" ON achievements;
CREATE POLICY "authenticated_read" ON achievements
  FOR SELECT
  TO authenticated
  USING (true);

-- progress: cada user vê/escreve só o próprio; admin lê tudo
DROP POLICY IF EXISTS "progress_student_rw" ON progress;
DROP POLICY IF EXISTS "student_rw" ON progress;
DROP POLICY IF EXISTS "admin_read" ON progress;
...
DROP POLICY IF EXISTS "admin_read_progress" ON progress;
CREATE POLICY "admin_read_progress" ON progress
  FOR SELECT
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- notes: cada user vê/escreve só as próprias; admin lê tudo
DROP POLICY IF EXISTS "notes_student_rw" ON notes;
DROP POLICY IF EXISTS "student_rw" ON notes;
DROP POLICY IF EXISTS "admin_read" ON notes;
...
DROP POLICY IF EXISTS "admin_read_notes" ON notes;
CREATE POLICY "admin_read_notes" ON notes
  FOR SELECT
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- videos: cada user vê/escreve só os próprios; admin lê tudo
DROP POLICY IF EXISTS "videos_student_rw" ON videos;
DROP POLICY IF EXISTS "student_rw" ON videos;
DROP POLICY IF EXISTS "admin_read" ON videos;
DROP POLICY IF EXISTS "own_videos" ON videos;
...
DROP POLICY IF EXISTS "admin_read_videos" ON videos;
CREATE POLICY "admin_read_videos" ON videos
  FOR SELECT
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- unlocked_achievements: cada user vê/escreve só os próprios; admin lê tudo
DROP POLICY IF EXISTS "unlocked_achievements_student_rw" ON unlocked_achievements;
DROP POLICY IF EXISTS "student_rw" ON unlocked_achievements;
DROP POLICY IF EXISTS "admin_read" ON unlocked_achievements;
DROP POLICY IF EXISTS "own_unlocked_achievements" ON unlocked_achievements;
CREATE POLICY "own_unlocked_achievements" ON unlocked_achievements
  FOR ALL
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "admin_read_unlocked_achievements" ON unlocked_achievements;
CREATE POLICY "admin_read_unlocked_achievements" ON unlocked_achievements
  FOR SELECT
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- project_completions: cada user vê/escreve só os próprios; admin lê tudo + pode UPDATE
DROP POLICY IF EXISTS "project_completions_student_rw" ON project_completions;
DROP POLICY IF EXISTS "student_rw" ON project_completions;
DROP POLICY IF EXISTS "admin_read" ON project_completions;
DROP POLICY IF EXISTS "own_project_completions" ON project_completions;
CREATE POLICY "own_project_completions" ON project_completions
  FOR ALL
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "admin_read_project_completions" ON project_completions;
CREATE POLICY "admin_read_project_completions" ON project_completions
  FOR SELECT
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

-- project_completions: admin pode UPDATE (para aprovar/reprovar missões)
DROP POLICY IF EXISTS "project_completions_admin_update" ON project_completions;
DROP POLICY IF EXISTS "admin_update" ON project_completions;
CREATE POLICY "admin_update" ON project_completions
  FOR UPDATE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'))
...
-- ============================================================
-- 6. UPDATE DE CONTEÚDO DAS 40 AULAS
-- Identifica pelo título de cada aula
-- ============================================================
...
<ul>
  ...
  <li><strong>Cada ferramenta tem um poder diferente:</strong> ChatGPT, Claude, Midjourney, Cursor — cada uma tem seu ponto forte.</li>
...
-- Aula 2: Suas ferramentas de batalha
...
<p><strong>ChatGPT</strong> é a sua espada principal. Faz de tudo: escreve texto, ajuda com código, responde perguntas, cria ideias. É a ferramenta mais versátil e tem versão gratuita robusta. <strong>Claude</strong> é o cajado de magia: raciocina mais fundo, escreve com mais qualidade, é ótimo para projetos longos. <strong>Cursor e Windsurf</strong> são a armadura especial — você escreve o que quer criar e o código aparece em tempo real no seu computador. <strong>Bolt e v0</strong> são portais instantâneos: você descreve um site e ele aparece em segundos, pronto para usar.</p>

<p>A boa notícia: você <strong>não precisa pagar por todas</strong>. Comece com o ChatGPT gratuito para quase tudo. O Cursor já está no seu computador. Ao longo do curso, você vai conhecendo cada ferramenta na prática.</p>

<ul>
  <li><strong>ChatGPT:</strong> o mais versátil, tem versão gratuita poderosa.</li>
  <li><strong>Claude:</strong> melhor para escrita longa e raciocínio complexo.</li>
  <li><strong>Cursor/Windsurf:</strong> para criar e editar código enquanto você conversa.</li>
  <li><strong>Bolt/v0:</strong> para criar interfaces e sites com um único prompt.</li>
  <li><strong>Cada ferramenta tem limite de uso gratuito</strong> — aprenda a gerenciar.</li>
</ul>

<p><strong>Exemplo de prompt para testar o ChatGPT:</strong><br>
<em>"Você é meu assistente pessoal de criação. Me dê 3 ideias criativas de projetos que um estudante de 14 anos poderia criar usando IA, listando qual ferramenta usar em cada uma."</em></p>
$aula2$ WHERE title ILIKE '%ferramentas de batalha%' OR title ILIKE '%suas ferramentas%';

-- Aula 3: Como a IA "pensa"
...
<p>Agora abra o ChatGPT (chat.openai.com) e faça seu primeiro pedido de verdade. Não tenha medo de errar — você pode refazer quantas vezes quiser. O importante é começar. Cada prompt que você escreve é um treino para se tornar um mestre.</p>
...
-- Aula 5: O que a IA não consegue fazer
...
-- Aula 1: Prompt ruim vs Prompt bom
UPDATE lessons SET content = $aula6$
<p>Dar missão para um NPC do Blox Fruits é fácil — você clica e tem 3 opções no máximo. Mas dar missão para um jogador humano de alto nível? Você explica o contexto, o objetivo, a recompensa, o prazo, tudo. Com a IA é exatamente assim: um prompt vago dá resultado de NPC. Um prompt detalhado desbloqueia o verdadeiro poder.</p>
...
-- Aula 4: Pedindo revisões e melhorias
...
-- Aula 5: Dividindo problemas grandes
...
<p>Quando você joga tudo em um único prompt gigantesco — "cria um site completo com sistema de login, pagamentos, blog, loja e chat ao vivo" — a IA vai tentar fazer tudo ao mesmo tempo e vai perder qualidade em cada parte. A técnica certa é <strong>quebrar o problema em fases</strong>. Primeiro peça o plano. Depois execute fase por fase.</p>
...
-- Aula 6: Seu grimório de prompts
...
-- Aula 1: Como um site funciona
UPDATE lessons SET content = $aula12$
<p>Um personagem do One Piece tem três partes essenciais: o <strong>esqueleto</strong> (a estrutura do corpo — ossos do Brook), a <strong>roupa</strong> (a aparência, o estilo — as roupas de cada tripulante), e as <strong>habilidades</strong> (os poderes especiais que os diferenciam). Um site funciona exatamente assim, com três camadas: HTML, CSS e JavaScript.</p>
...
-- Aula 2: O que é uma landing page
...
<p>Uma boa landing page tem: seção hero (título impactante + imagem + botão), seção de problema/solução, seção de serviços ou produtos, prova social (depoimentos), e um CTA final (Call to Action — o botão de ação). É essa estrutura que você vai aprender a criar com IA.</p>
...
-- Aula 3: Como pedir um site completo à IA
...
-- Aula 6: Como publicar no ar com Vercel
...
-- Aula 7: Como conectar um domínio ao site
...
<p>O processo passo a passo: <strong>1)</strong> No Vercel, acesse seu projeto e clique em "Settings" (Configurações). <strong>2)</strong> Vá em "Domains" e clique em "Add". <strong>3)</strong> Digite seu domínio (ex: seusite.com.br) e clique em Add. <strong>4)</strong> O Vercel vai mostrar registros DNS — são como "instruções de redirecionamento" que você precisa copiar. <strong>5)</strong> Acesse o painel do lugar onde comprou o domínio (ex: Registro.br), vá em "DNS" ou "Zona de DNS", e cole os registros que o Vercel forneceu. <strong>6)</strong> Aguarde entre 1 e 24 horas para a propagação acontecer.</p>
...
-- Aula 2: Tipos de dados que um painel pode mostrar
...
<p><strong>KPIs</strong> (números grandes em destaque) são para os indicadores mais importantes: total de vendas, número de clientes, receita do mês. <strong>Gráficos de linha</strong> mostram evolução no tempo — como as vendas cresceram semana a semana. <strong>Gráficos de barra</strong> são para comparações — quais produtos vendem mais. <strong>Gráficos de pizza</strong> mostram proporções — quanto cada categoria representa do total. <strong>Tabelas</strong> mostram detalhes — lista dos últimos pedidos com nome, valor e status.</p>
...
-- Aula 3: Como pedir um painel à IA
...
<p>Um exemplo completo: <em>"Crie um dashboard HTML em arquivo único para uma loja fictícia de eletrônicos. Métricas para mostrar: 1) Total de vendas do mês (R$45.230 — KPI grande), 2) Produtos mais vendidos: iPhone, Samsung Galaxy, AirPods, Fone JBL — gráfico de barras horizontal, 3) Evolução de vendas dos últimos 7 dias — gráfico de linha, 4) Clientes novos este mês: 127 — KPI com comparação ao mês anterior (+12%), 5) Tabela com os 5 últimos pedidos (data, produto, valor, status). Estilo escuro moderno, acentos em azul elétrico. Responsivo para mobile."</em></p>
...
<p><strong>Exemplo de prompt que você pode usar:</strong><br>
<em>"Crie um dashboard HTML para um personal trainer com dados fictícios mostrando: número de alunos ativos (KPI), evolução de matrículas nos últimos 6 meses (gráfico de linha), distribuição de alunos por modalidade — musculação, funcional, online (gráfico de pizza), e lista dos últimos 5 pagamentos recebidos (tabela). Tema escuro, cores verde e branco."</em></p>
$aula21$ WHERE title ILIKE '%como pedir um painel%' OR title ILIKE '%painel à ia%';

-- Aula 4: O que é uma API
...
-- Aula 5: Como usar dados reais no dashboard
UPDATE lessons SET content = $aula23$
<p>Você já sabe criar dashboards com dados fictícios. Agora é hora de conectar dados reais. E a melhor notícia: existe uma API pública, gratuita, sem necessidade de cadastro, que qualquer pessoa pode usar agora mesmo — a <strong>API do Banco Central do Brasil</strong>. Com ela, você consegue pegar o preço do dólar, do euro, do real em tempo real. E a IA sabe exatamente como conectar essa API ao seu dashboard.</p>
...
-- Aula 1: Tipos de jogos que a IA cria bem
...
-- Aula 2: Como descrever mecânicas de jogo
UPDATE lessons SET content = $aula25$
<p>A diferença entre um jogo medíocre e um jogo divertido está nas mecânicas. <strong>Mecânica</strong> é o que acontece quando o jogador faz alguma coisa. É a regra de causa e efeito: <em>[ação do jogador] → [reação do jogo]</em>. E quanto mais detalhadamente você descreve as mecânicas para a IA, mais o jogo funciona como você imaginou.</p>
...
-- Aula 3: Como pedir sprites e assets
...
<p>Para criar imagens de jogos, as melhores ferramentas são: <strong>DALL-E</strong> (dentro do ChatGPT Plus — mais fácil), <strong>Leonardo.ai</strong> (gratuito e especializado em jogos), e <strong>Midjourney</strong> (pago, mas os resultados são incríveis para assets de jogos). Para sons, use: <strong>ElevenLabs</strong> para vozes e narrações, <strong>Suno</strong> para músicas de fundo, e <strong>Freesound.org</strong> para efeitos sonoros gratuitos.</p>
...
-- Aula 4: Como hospedar um jogo simples
...
-- Aula 5: Como fazer updates e melhorias
...
-- Aula 2: O que é um banco de dados
...
<ul>
  ...
  <li><strong>Backend = o que processa</strong> por baixo — verificações, cálculos, banco de dados.</li>
...
-- Aula 4: Como descrever um sistema para a IA
...
-- Aula 5: O que é autenticação
UPDATE lessons SET content = $aula33$
<p>Antes de cada chamada no Den Den Mushi, o sistema verifica quem está ligando. Alguns snails bloqueiam chamadas não autorizadas. Isso é autenticação — o sistema que confirma <strong>quem você é</strong> antes de te deixar entrar. Em sistemas digitais, autenticação é o sistema de login: você prova que é você, e o sistema te dá acesso.</p>
...
-- Aula 1: Como planejar um projeto do zero
...
-- Aula 2: Como apresentar para um cliente
...
-- Aula 3: Como precificar seu trabalho
...
<p>Tabela de referência para iniciantes: <strong>Landing page simples</strong> (1 página, sem sistema): R$200–500. <strong>Site completo</strong> (3-5 páginas, sem sistema): R$500–1.500. <strong>Sistema simples</strong> (cadastro, agendamento, com banco de dados): R$1.000–3.000. Como calcular: quanto tempo você levou × valor da sua hora. Para iniciante: R$30–50 por hora. Para quem já tem portfólio: R$80–150.</p>
...
-- Aula 4: Como entregar e passar o projeto
...
-- Aula 5: Como lidar com revisões do cliente
